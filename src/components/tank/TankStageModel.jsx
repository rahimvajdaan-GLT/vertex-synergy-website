...
// Loads a Tank GLB, clones it (with cloned materials so opacity can be driven
// per-model), normalises it to a 1-unit max dimension centred on x/z with its
// base on y=0, and mounts it imperatively via a pivot Object3D. Optional config
// lets us nudge individual stages after inspecting the real models.
export default function TankStageModel({
  url,
  state,
  groundY = -1.5,
  float = 0,
  ...
  const { pivot, materials } = useMemo(() => {
    ...
    // three stages share one centre + ground level.
